...
class PAAC(torch.nn.Module):
    def __init__(self, config, data):
        ...
        # Now meta-learn lambda2 and gamma
        self.lambda2 = torch.nn.Parameter(torch.tensor(float(config.lambda2)))
        self.gamma = torch.nn.Parameter(torch.tensor(float(config.gamma)))
    ...
    def cl_loss(self, u_idx, i_idx, j_idx):
        ...
        item_cl_pop = torch.clamp(self.gamma, 0, 1) * metrics.InfoNCE_i(
            item_v1[batch_pop], item_v2[batch_pop], item_v2[batch_unpop],
            self.temperature, torch.clamp(self.lambda2, 0, 10)
        )

        item_cl_unpop = (1 - torch.clamp(self.gamma, 0, 1)) * metrics.InfoNCE_i(
            item_v1[batch_unpop], item_v2[batch_unpop], item_v2[batch_pop],
            self.temperature, torch.clamp(self.lambda2, 0, 10)
        )
