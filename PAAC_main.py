...
Tmin = 0.2  # for popular items (sharper)
Tmax = 0.7 # for unpopular items (softer)
...
class PAAC(torch.nn.Module):
    def __init__(self, config, data):
        ...
        self.item_temperatures = self.compute_dynamic_temperatures()
    ...
    # def compute_dynamic_temperatures(self): ##Linear scaling
    # # Ensure all item IDs in self.pop_train have counts
    #     popularity = self.pop_train
    #     pop_values = popularity
    #     pop_min = min(pop_values)
    #     pop_max = max(pop_values)

    #     item_temperatures = {}
    #     for item_id, count in enumerate(popularity):
    #         if pop_max == pop_min:
    #             temp = (Tmin + Tmax) / 2
    #         else:
    #             temp = Tmin + (Tmax - Tmin) * (count - pop_min) / (pop_max - pop_min)
    #         item_temperatures[item_id] = temp
    #     return item_temperatures

    def compute_dynamic_temperatures(self): ##non-linear scaling using sigmoid
    
        alpha = 1.0  # You can tune this hyperparameter

        popularity = self.pop_train
        pop_values = popularity
        pop_median = np.median(pop_values)  # Use median for sigmoid center

        item_temperatures = {}
        for item_id, count in enumerate(popularity):
            sigmoid_component = 1 / (1 + math.exp(-alpha * (count - pop_median)))
            temp = Tmin + (Tmax - Tmin) * sigmoid_component
            item_temperatures[item_id] = temp
        return item_temperatures
    ...
    def cl_loss(self, u_idx, i_idx, j_idx):
        ...
        # item_cl_unpop=(1-self.gamma)*metrics.InfoNCE_i(item_view_1[batch_unpop], item_view_2[batch_unpop],item_view_2[bacth_pop], self.temperature,self.lambda2)
                # Gather adaptive temperature for each item
        temp_pop = torch.tensor([self.item_temperatures.get(int(i), Tmax) for i in bacth_pop]).to(self.device)
        temp_unpop = torch.tensor([self.item_temperatures.get(int(i), Tmax) for i in batch_unpop]).to(self.device)

        item_cl_pop = self.gamma * metrics.InfoNCE_i(
            item_view_1[bacth_pop], item_view_2[bacth_pop], item_view_2[batch_unpop],
            temp_pop, self.lambda2
        )
        item_cl_unpop = (1 - self.gamma) * metrics.InfoNCE_i(
            item_view_1[batch_unpop], item_view_2[batch_unpop], item_view_2[bacth_pop],
            temp_unpop, self.lambda2
        )
